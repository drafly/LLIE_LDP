...
class ELDModelBase(BaseModel):
    def set_input(self, data, mode='train'):
        ...
        mode = mode.lower()
        if mode == 'train':
            input, target, variance, self.ratio, self.K = data['input'], data['target'], data['variance'], data["ratio"], data["K"]
        elif mode == 'eval':
            input, target, variance, data_name, self.ratio, self.K = data['input'], data['target'], data['variance'],data['fn'], data["ratio"], data["K"].item()
        elif mode == 'test':
        ...
        if len(self.gpu_ids) > 0:
            ...
            if variance is not None:
                variance = variance.to(device=self.gpu_ids[0])
    ...
    def eval(self, i, data, savedir=None, suffix=None, correct=False, crop=True, frame_id=None, iter_num=0, old_diffusion=False,
             res_in=None ):
        ...
        with torch.no_grad():
            ...
            if not old_diffusion:
                ...
                output_list = self.forward(iter_num=iter_num, resid_output=resid_output)
            ...
            variance = self.input - self.target
            ...
            if savedir is not None:
                ...
                if self.rawpath:
                    if self.cfa == 'bayer':
                        ...
                        resid_output = tensor2im(postprocess_bayer_v2(self.rawpath, variance))
                ...
                if self.data_name is not None:
                    ...
                    if frame_id is not None:
                        ...
                        if suffix is not None:
                            ...
                            Image.fromarray(input.astype(np.uint8)).save(join(savedir, name, 'in_{}_{:.2f}_{:.2f}.png'.format(self.opt.model_path.split("/")[-2], res['PSNR'], res['SSIM'])))
                            # Image.fromarray(resid_output.astype(np.uint8)).save(join(savedir, name, 'gtrsid_{}_{:.2f}_{:.2f}.png'.format(self.opt.model_path.split("/")[-2], res['PSNR'], res['SSIM'])))
                            ...
                            # Image.fromarray(target.astype(np.uint8)).save(join(savedir, name, 'all_label.png'))
...
class ELDModel(ELDModelBase):
    ...
    def initialize(self, opt):
        ...
        self.netG = arch.__dict__[self.opt.netG](in_channels, out_channels, resid=opt.resid).to(self.device)
    ...
    def forward(self):
        ...
        if self.opt.chop:            
            output, _ = self.forward_chop(input_i)
        else:
            output, _ = self.netG(input_i)
        self.output = output  
        return output
