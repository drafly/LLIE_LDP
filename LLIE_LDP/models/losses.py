...
def init_loss(opt):
    ...
    pixel_loss_resid = ContentLoss()
    pixel_loss_resid_all = ContentLoss()
    ...
    if opt.loss == 'l1':
        pixel_loss_resid.initialize(nn.L1Loss())
    elif opt.loss == 'l2':
        pixel_loss_resid.initialize(nn.MSELoss())
    elif opt.loss == 'kl':
        pixel_loss_resid.initialize(KL_Loss())

    loss_dic['pixel_resid'] = pixel_loss_resid

    if opt.resid_loss == 'l1':
        pixel_loss_resid_all.initialize(nn.L1Loss())
    elif opt.resid_loss == 'l2':
        pixel_loss_resid_all.initialize(nn.MSELoss())
    elif opt.resid_loss == 'kl':
        pixel_loss_resid_all.initialize(KL_Loss())

    loss_dic['pixel_loss_resid_all'] = pixel_loss_resid_all
