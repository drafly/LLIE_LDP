...
class NoiseModelBase:  # base class
    def __call__(self, y, params=None, continuous=False):
        ...
        if 'P' in self.model:
            ...
            poisson_noise_variance_P = np.mean(y / K)
        ...
        if 'r' in self.model: # row noise
            ...
            row_noise_variance = sigma_r
            noise_levels['row'] = row_noise_variance
        if 'g' in self.model:
            z = z + np.random.randn(*y.shape).astype(np.float32) * np.maximum(g_scale, 1e-10)  # Gaussian noise
            gaussian_noise_variance_g = g_scale
            noise_levels['gaussian_g'] = gaussian_noise_variance_g
            ...
            # lam = np.random.choice(G_shape)
            gaussian_noise_variance_G = np.var(tukeylambda.rvs(lam, size=y.shape).astype("float32")) * G_scale
            noise_levels['gaussian_G'] = gaussian_noise_variance_G
        else:
            z = z
        ...
        # total_noise_variance = (tmp - x)
        total_noise_variance = (tmp - x).astype("float32")
        total_noise_variance = np.clip(total_noise_variance, 0, 1)
        # print(total_noise_variance.shape)

        return z, total_noise_variance, {"K": K, "ratio": ratio, "g_scale": g_scale, "saturation_level": saturation_level}
