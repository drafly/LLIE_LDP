...
def create_lmdb_train(
    ...
    with env.begin(write=True) as txn:
        ...
        for i, fn in enumerate(fns):
            ...
            for j in range(N):
                meta_info[k] = (wb, ccm, iso, ratios[i] if ratios is not None else 0)
                data = X[j].tobytes()
                str_id = '{:08}'.format(k)
                k += 1
...
def create_lrd_dataset_paired(num_samples=None):  # paired real data
    print('create ldr dataset...')
    # sourcedir = '/media/kaixuan/DATA/Papers/Code/Data/Raw/SID/Sony/'

    fns = sorted(read_paired_fns('./dataset/LDR_train.txt'))
    if num_samples is not None:
        fns = fns[:num_samples]

    ratios = [compute_expo_ratio_LRD(fn[0], fn[1], fn[2], fn[3]) for fn in fns]

    create_lmdb_train(
        [join(sourcedir, 'short', fn[0]) for fn in fns], join(destdir, 'LDR_input_Raw'),
        ksize=(4, 512, 512),
    ...
    create_lmdb_train(
        [join(sourcedir, 'long', fn[1]) for fn in fns], join(destdir, 'LDR_target_Raw'),
        ksize=(4, 512, 512),
        stride=(4, 512, 512),
        srgb=False,
    )
...
def create_sony_dataset_paired(num_samples=None): # paired real data
    ...
    create_lmdb_train(
        [join(sourcedir, 'short', fn[0]) for fn in fns], join(destdir, 'SID_Sony_input_Raw_'),
        ksize=(4, 512, 512),
        stride=(4, 512, 512), ratios=ratios,
        srgb=False,
    )
