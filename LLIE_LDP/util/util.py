...
def get_model_list(dirname, key, epoch=None):
    ...
    epoch_index = [int(os.path.basename(model_name).split('_')[-2]) for model_name in gen_models if
                   os.path.basename(model_name).split('_')[-2].isdigit()]
...
def auto_padding(img, scale):
    # _, _, y, x = img.shape
    # end_y = y % scale
    # end_x = x % scale
    # return img[:, :, :-end_y if end_y!=0 else None, :-end_x if end_x!=0 else None]
    # # img: numpy image with shape H*W*C
    _, _, w, h = img.shape
    times = scale
    h1, w1 = (times - h % times) // 2, (times - w % times) // 2
    h2, w2 = (times - h % times) - h1, (times - w % times) - w1
    img = F.pad(img, [h1, h2, w1, w2], mode="reflect")
...
def write_loss(writer, prefix, avg_meters, iteration, log_to_file):
    for key in avg_meters.keys():
        meter = avg_meters[key]
        writer.add_scalar(
            os.path.join(prefix, key), meter, iteration)
        if log_to_file:
            with open('loss_values.file', 'a') as f:
                f.write(f"{iteration},{key},{meter}\n")  # 写入迭代次数、键和值
